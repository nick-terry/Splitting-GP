...
def makeLocalGPModels(kernelClass,likelihood,w_gen,k,**kwargs):
    ...
    for i in range(k):
        if 'maxChildren' in kwargs:
            models.append(makeModel(kernelClass, likelihood, w_gen, maxChildren=kwargs['maxChildren']))
        else:
...
def runCrossvalidationExperiment(modelType,**kwargs):
    ...
    if modelType=='exact':
        def getModelsFunction(): return makeLocalGPModels(kernel, likelihood, 0, k, maxChildren=1)
