...
model = LocalGP.LocalGPModel(likelihood,kernel,w_gen=.2)
...
z = 5*torch.sin(xyGrid[:,:,0]**2+(2*xyGrid[:,:,1])**2).reshape((25,25,1))
...
plt.colorbar()
plt.scatter(xyGrid[0,0,0].detach(),xyGrid[0,0,1].detach())
plt.scatter(xyGrid[9,12,0].detach(),xyGrid[9,12,1].detach())