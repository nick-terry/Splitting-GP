# -*- coding: utf-8 -*-
"""
Created on Fri Jan 23 13:23:10 2015
...
class gPoE(GP):
        ...
        if cmethod.startswith('rep'):
            ...
            for i in range(c): 
                ...
                if len(next_levels) == 0:
                    ...
                    child = gPoE(subset.X,subset.y,cov_type=self.cov_type,
    ...
    def predict(self,Xp,variance=False,latent_variance=False,entropy=False):
        ...
        if hasattr(self, 'normalize'):
          if self.normalize:
            betaChildren = betaChildren/np.sum(betaChildren, axis=1).reshape(betaChildren.shape[0],1)
...
#        ymu = np.sum(preds*weights_norm,axis=1)
        ...
        S3 = S2/betaChildren
        precGPOE = np.sum(1.0/S3,axis=1)
        SGPOE = 1.0/precGPOE
        ymu =SGPOE*np.sum(preds/S3,axis=1)

        beta = np.sum(betaChildren, axis = 1)
