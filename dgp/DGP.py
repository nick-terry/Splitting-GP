...
class DGP(GP):
        ...
        if cmethod.startswith('rep'):
            ...
            for i in range(c): 
                ...
                if len(next_levels) == 0:
                    ...
                    child = DGP(subset.X,subset.y,cov_type=self.cov_type,
    ...
    def predict(self,Xp,variance=False,latent_variance=False,entropy=False):
        ...
        # collect mean/variance predictions from children
        preds = np.vstack(predictions_ymu).T
        S2 = np.vstack(predictions_fs2).T
        
        # compute mean/variance of the subtree starting at this node
        precDGP = np.sum(1/S2,axis=1)
        SDGP = 1/precDGP
        ymu =SDGP*np.sum(preds/S2,axis=1)
        ...
        if variance or latent_variance:
            ...
            if entropy:
                # compute the difference between prior and predictive variance
