...
class gBCM(GP):
        ...
        if cmethod.startswith('rep'):
            ...
            for i in xrange(c):
                ...
                if len(next_levels) == 0: # GP at the leaves
                    ...
                    else: # all other levels must be PoEs (HGPs)
    ...
    def predict(self,Xp,variance=False,latent_variance=False,entropy=False):
        ...
        precgBCM = np.sum(1.0/S2,axis=1)
        if hasattr(self, 'correction'):
          if self.correction:
            precgBCM += (1.0-np.sum(betaChildren,axis=1))/np.exp(self.params[-2])
        ...
        SgBCM = 1.0/precgBCM
        ymu =SgBCM*np.sum(preds/S2,axis=1)
        ###########
