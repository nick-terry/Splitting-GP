...
def evalModel(w_gen,numSamples):
    ...
    def makeModel(kernelClass,likelihood,w_gen):
        ...
        model = SplittingLocalGP.SplittingLocalGPModel(likelihood,kernelClass(ard_num_dims=2),
                                                       splittingLimit=3,inheritKernel=False)
        return model
    ...
    for randPairIndex in range(numSamples):
        ...
        if j == 10: #this is where the error occurs
            closestChildIndex,minDist = model.getClosestChild(x_train)
            reprod_model = model.children[closestChildIndex]
            return reprod_model,x_train,y_train
...
for numSamples in numSamplesVals:
    reprod_model,reprod_x,reprod_y = evalModel(.5,numSamples)
print('done')
